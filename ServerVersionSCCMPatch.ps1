...
$computers = Import-Csv AllServers.csv
foreach ($cpu in $computers){
    Function Get-EsetInfo{
            param ($aComputer)
            #Receive the password expire date and convert it to normal time.
            $esetFind = Get-CimInstance -ComputerName $cpu.Computer -ClassName win32_product | Select-Object name,Version | Where-Object {$_.name -eq "Eset File Security"} | Select-Object version
    ...
    Function Get-trapsFind{
            param ($aComputer)
            #Receive the password expire date and convert it to normal time.
            $trapsFind = Get-CimInstance -ComputerName $cpu.Computer -ClassName win32_product | Select-Object name,Version | Where-Object {$_.name -eq "Traps 4.1.3.33176" } | Select-Object version
            $trapsVersion = $trapsFind.version.toString()
            return $trapsVersion
            }
    Function Test-PCConnection{
            param ($aComputer)
            $test = Test-Connection -ComputerName $cpu.Computer -BufferSize 16 -Count 1 -ea 0 -Quiet
            return $test
            }

    $sheet.Cells.Item($row,$column) = $cpu.Computer
    $runCodeOrNaw = Test-PCConnection $cpu.Computer

    if ($runCodeOrNaw -eq "True"){
        try {
            $column++
            $esetInfomation = Get-EsetInfo $cpu.Computer
            $trapsInformation = Get-trapsFind $cpu.Computer
